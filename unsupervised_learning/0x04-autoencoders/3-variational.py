...
def autoencoder(input_dims, hidden_layers, latent_dims):
    ...
    def sampling(args):
        ...
        epsilon = keras.backend.random_normal(shape=(keras.backend.shape(mean)[0], latent_dims), mean=0, stddev=0.1)

        return mean + keras.backend.exp(log_sigma) * epsilon

    z = keras.layers.Lambda(sampling, output_shape=(latent_dims,))([mean, log_sigma])
    ...
    dec_output_layer = keras.layers.Dense(units=input_dims, activation="sigmoid")(dec)
    # define encoder
    ...
    # define loss
    # def kl_divergence(inputs, outputs, input_dims, log_sigma, mean):
    """"""
    rec_loss = keras.losses.binary_crossentropy(inputs, outputs)
    rec_loss *= input_dims
    kl_loss = 1 + log_sigma - keras.backend.square(mean) - keras.backend.exp(log_sigma)
