...
class Yolo:
    ...
    def preprocess_images(self, images):
        ...
        return np.asarray(pimages, dtype=np.float32), np.asarray(image_shapes)
    ...
    def predict(self, folder_path):
        ...
        for i, image in enumerate(images):
            print(img_paths[i])
            pred = [x[i] for x in predictions]
            boxes, box_confs, box_cls_prob = self.process_outputs(pred, image_shapes[0])
            filtered_boxes, box_cls, box_scores = self.filter_boxes(boxes, box_confs, box_cls_prob)
            pbox, pcls, pscore = self.non_max_suppression(filtered_boxes, box_cls, box_scores)
            self.show_boxes(images[i], pbox, pcls, pscore, img_paths[i])
        return ([pbox, pcls, pscore], img_paths)
