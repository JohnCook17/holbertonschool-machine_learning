...
def ngram_bleu(references, sentence, n):
    ...
    for ref in references:
        for i in range(1, len(weights) + 1):
            ...
            ref_lengths += (np.abs(
                            np.asarray([len(x)
                                        for x in references]) - len(sentence))
                            ).argmin()

    best = (np.abs(np.asarray([len(x)
                               for x in references]) - len(sentence))
            ).argmin()

    # print(p_num_total, p_den_total)
    # print(sen_lens, ref_lengths)
